...
> 注意：免费 API Key 每日调用上限 5000 次，项目 711 个景点可一天内完成全量更新。
...
- **写入前必须验证**：添加或修改数据来源链接前，用 `curl` 测试确认返回 HTTP 状态码 **必须为 200**，其他任何状态码（包括 301/302 重定向、403、404、000 等）均视为不可用：
...
- **避免百度百科歧义页面**：部分短格式百度百科链接（如 `https://baike.baidu.com/item/珠海市`）可能指向**多义词/歧义页面**，只有词条索引而无实际内容。写入前必须在浏览器中打开确认页面有实际内容，而非仅显示「多义词」提示。获取方法：在百度搜索 `景点名 site:baike.baidu.com`，点击第一个结果即可获得有效 URL。
